...
{
  MODBUS_HR[MBHR_DISCRETE_OUTPUTS_LOW] = inPack->data[0];
...
/*void analise_modbus(ComMessage* inPack, ComMessage* outPack)
{
  ...
  case 6:		// <06> Запись одного регистра хранения.
    ...
    break;
  case 8:   // <06> Запись одного регистра хранения.
...
void ModbusClient::send_writereg_06(uint8_t addr, uint16_t reg,uint16_t val)
{
  uint8_t data[255], rbuf[255];
  data[0] = addr;
  data[1] = 0x06;
  data[2] = (reg & 0xff00) >> 8;
  data[3] = reg & 0x00ff;
...
//------------------------------------------------------------------------------
void ModbusClient::send_loop_08(uint8_t addr)
{
  uint8_t buf[4], rbuf[255];
  buf[0] = addr;
  buf[1] = 0x08;
...
//------------------------------------------------------------------------------
void ModbusClient::send_read_03(uint8_t addr, uint16_t reg,uint16_t cnt)
{
  uint8_t data[255]={0};
  data[0] = addr;
  data[1] = 0x03;
